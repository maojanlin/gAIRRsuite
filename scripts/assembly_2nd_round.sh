...
bwa mem -t 16 $outer_dir"asm_contigs/TCRV_contigs_"$cluster_id".fasta" $outer_dir"asm_reads_and_alleles/TCRV_"$cluster_id"_H1.fasta" $outer_dir"asm_reads_and_alleles/TCRV_"$cluster_id"_H2.fasta" > $outer_dir"asm_back_align/TCRV_realign_"$cluster_id".sam"
...
python3 ../../immunogenomics/scripts/parse_contig_realign.py -fs $outer_dir"asm_back_align/TCRV_realign_"$cluster_id".sam" -fo $outer_dir"asm_back_align/TCRV_remain_"$cluster_id".fasta" > $outer_dir"asm_back_align/TCRV_realign_"$cluster_id".rpt" -it_region $interested_region --contig_file $outer_dir"asm_contigs/TCRV_contigs_"$cluster_id".fasta" --allele_file $outer_dir"asm_reads_and_alleles/TCRV_"$cluster_id"_allele.fasta" --corrected_contig_output_file $outer_dir"asm_alignment/corrected_contig_size300.fasta"
